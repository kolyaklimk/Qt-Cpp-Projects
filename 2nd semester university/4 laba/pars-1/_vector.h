...
public:
    ...
    _Vb_val(int* ptr, int pos) : value_ptr(ptr), position(pos) {}

    _Vb_val& operator= (bool rhs) {
        rhs ? *value_ptr |= ((int)1 << position) :
            *value_ptr &= ~((int)1 << position);
        return *this;
    }

    operator bool() const {
        return *value_ptr & ((int)1 << position);
    }
};
...
public:
    ...
    vector(short amount, bool value) :size(amount), capacity(default_capacity), arr(new int[capacity]) {
